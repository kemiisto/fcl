subroutine SPECIFIC_PROCEDURE(unit, array, format, max_columns, headers, decorate)
  ...
  SPECIFIC_TYPE(kind=SPECIFIC_KIND), dimension(:,:), intent(in) :: array
  ...
  read (format(2:2),*) format_width

  write (row_format, "(i2,a,a)") max_columns, format, ")"
  write (header_format, "(i2,a,i2,a)") max_columns, "i", format_width, ")"
  ...
  do while (left_column <= columns)
    if (headers) then
      ...
      if (decorate) then
        write (unit, "(a)") repeat(" ", row_header_width + 2)//repeat("-", format_width * (right_column - left_column + 1))
      else
        write (unit, "(bn)")
      end if
    else
      if (decorate) then
        write (unit, "(a)") "  "//repeat("-", format_width * (right_column - left_column + 1))
      end if
    end if
